...
sudo pacman -S --noconfirm --needed xmobar feh xorg xorg-xinit xorg-xinput $DRI xmonad

# install fonts
mkdir -p ~/.local/share/fonts
mkdir -p ~/.srcs

cp -r ~/archbuildplayer/fonts/* ~/.local/share/fonts/
fc-cache -f
clear 
...
# git clone https://gist.github.com/f70dea1449cfae856d42b771912985f9.git ./picom 
    if [ -d ~/.config/rofi ]; then
        ...
        cp -r ~/archbuildplayer/.config/rofi/* ~/.config/rofi/;
    else
        echo "Installing rofi configs..."
        mkdir ~/.config/rofi && cp -r ~/archbuildplayer/.config/rofi/* ~/.config/rofi/;
    fi
    sleep 5
    ...
    if [ -d ~/.config/eww ]; then
        ...
        cp -r ~/archbuildplayer/.config/eww/* ~/.config/eww/;
    else
        echo "Installing eww configs..."
        mkdir ~/.config/eww && cp -r ~/archbuildplayer/.config/eww/* ~/.config/eww;
    fi
    if [ -f ~/.config/picom.conf ]; then
        echo "Picom configs detected, backing up..."
        cp -r ~/.config/picom.conf ~/.config/picom.conf.old;
        cp -r ~/archbuildplayer/.config/picom.conf ~/.config/picom.conf;
    else
    ...
    if [ -f ~/.config/alacritty.yml ]; then
        ...
        cp ~/archbuildplayer/.config/alacritty.yml ~/.config/alacritty.yml;
    else
    ...
    if [ -d ~/.config/dunst ]; then
        ...
        cp -r ~/archbuildplayer/.config/dunst/* ~/.config/dunst/;
    else
        echo "Installing dunst configs..."
        mkdir ~/.config/dunst && cp -r ~/archbuildplayer/.config/dunst/* ~/.config/dunst/;
    fi
    if [ -d ~/wallpapers ]; then
        echo "Adding wallpaper to ~/wallpapers..."
        cp -r ~/archbuildplayer/wallpapers/* ~/wallpapers/;
    else
        echo "Installing wallpaper..."
        mkdir ~/wallpapers && cp -r ~/archbuildplayer/wallpapers/* ~/wallpapers/;
    fi
   ...
   # fi
    if [ -d ~/.config/xmobar ]; then
        echo "XMobar configs detected, backing up..."
        mkdir ~/.config/xmobar && mv ~/.config/xmobar/xmobarrc ~/.config/xmobar/xmobarrc.bak
        cp -r ~/archbuildplayer/.config/xmobar/* ~/.config/xmobar/;
    else
        echo "Installing XMobar configs..."
        mkdir ~/.config/xmobar && cp -r ~/archbuildplayer/.config/xmobar/* ~/.config/xmobar/;
    fi
    ...
    if [ -d ~/.xmonad ]; then
        ...
        cp -r ~/archbuildplayer/xmonad.hs ~/.xmonad/;
    else
        echo "Installing xmonad configs..."
        mkdir ~/.xmonad && cp -r ~/archbuildplayer/xmonad.hs ~/.xmonad/;
    fi
    if [ -d ~/bin ]; then
        echo "~/bin detected, backing up..."
        mkdir ~/bin.old && mv ~/bin/ ~/bin.old/
        cp -r ~/archbuildplayer/bin/* ~/bin/;
	clear
    else
        echo "Installing bin scripts..."
        mkdir ~/bin && cp -r ~/archbuildplayer/bin/* ~/bin/;
	clear
